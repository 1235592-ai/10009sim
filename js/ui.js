window.UI = {
    ...
    togglePanel: function(id) {
        ...
        if(p.classList.contains('open')) { if(App.isPanelOpen) history.back(); } 
            ...
            document.querySelectorAll('.panel').forEach(el => el.classList.remove('open')); p.classList.add('open'); document.getElementById('overlay').classList.add('active');
    ...
    createCtrls: function(idx) {
        ...
        if(m.role==='ai') {
            ...
            if(m.variants.length>1) { const p = document.createElement('button'); p.innerText="<"; p.onclick=()=>{m.currentVariant=(m.currentVariant+m.variants.length-1)%m.variants.length; App.loadActiveRoom();}; const n = document.createElement('button'); n.innerText=">"; n.onclick=()=>{m.currentVariant=(m.currentVariant+1)%m.variants.length; App.loadActiveRoom();}; const s = document.createElement('span'); s.innerText=`${m.currentVariant+1}/${m.variants.length}`; c.append(p, s, n); }
        }
        const eb = document.createElement('button'); eb.innerText="수정"; eb.onclick=()=>this.enterEdit(idx); const rw = document.createElement('button'); rw.className = "btn-rewind"; rw.innerText="되감기"; rw.onclick=()=>App.rewindRoom(idx); const db = document.createElement('button'); db.innerText="삭제"; db.onclick=()=>{ if(!confirm("삭제하시겠습니까?")) return; r.history.splice(idx,1); Store.forceSave(); App.loadActiveRoom(); };
    ...
    enterEdit: function(idx) { 
        ...
        sBtn.onclick = () => { if(!confirm("수정하시겠습니까?")) return; if(editArea.value.trim()){ m.variants[m.currentVariant] = editArea.value; Store.forceSave(); App.loadActiveRoom(); } };
